...
class MyRedisCluster {
    ...
    setupClientListeners(client) {
        client.on("reconnecting", function () {
            ...
            appInsight.defaultClient.trackEvent({ name: "redisPubConnMsg", properties: propertySet });
            appInsight.defaultClient.trackMetric({ name: "redisPubReconnect", value: 1.0 });
        });
        ...
        client.on("error", (err) => {
            ...
            appInsight.defaultClient.trackEvent({ name: "redisPubConnError", properties: propertySet });
        });
        client.on("close", function () {
            var propertySet = {
                errorMessage: "Redis server connection closed"
            };
            appInsight.defaultClient.trackEvent({ name: "redisPubConnClosed", properties: propertySet });
            appInsight.defaultClient.trackMetric({ name: "redisPubConnClosed", value: 1.0 });
        });
    }
}
